            ...
            <div class="nettsmed-admin-cards">
                ...
                <div class="nettsmed-admin-card">
                    ...
                    <?php if (class_exists('ITSEC_Core')): ?>
                        <p><strong>2FA for Simple Admins, Editors, Authors & Contributors:</strong> 
                            <?php if ($disable_2fa): ?>
                                <span class="status-disabled">Disabled</span> - Simple admins, editors, authors and contributors can log in without 2FA
                            <?php else: ?>
                                <span class="status-enabled">Enabled</span> - Simple admins, editors, authors and contributors must use 2FA
                            <?php endif; ?>
                        </p>
                        <p class="description">Solid Security plugin detected. 2FA bypass is <?php echo $disable_2fa ? 'active' : 'inactive'; ?> for Simple Admins, Editors, Authors and Contributors.</p>
                    ...
                    <?php else: ?>
                        ...
                        <p class="description">Install the Solid Security plugin to enable 2FA bypass controls for Simple Admin, Editor, Author and Contributor users.</p>
...
function admin_dashboard_settings_page() {
    ...
    <div class="wrap">
        ...
        <p>Configure the security settings for the admin dashboard.</p>
                    ...
                    <td>
                        <?php if (class_exists('ITSEC_Core')): ?>
                            <label>
                                ...
                                Disable 2FA for Simple Admin, Editor, Author and Contributor users
                            </label>
                            <p class="description">When enabled, Simple Admin, Editor, Author and Contributor users will not be required to use Two-Factor Authentication (2FA). This setting is enabled by default for easier access.</p>
                        <?php else: ?>
...
function ns_disable_2fa_for_simpel_admin($providers, $user) {
    ...
    if ($disable_2fa_setting && $user instanceof WP_User) {
        // Check for simpel_admin, editor, author and contributor roles
        if (in_array('simpel_admin', (array)$user->roles, true) || 
